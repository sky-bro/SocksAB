...
std::string Cipher::deriveAeadSubkey(size_t length, const std::string &masterKey, const std::string &salt) {
  ...
  Botan::secure_vector<Botan::byte> skey = kdf->derive_key(length, reinterpret_cast<const uint8_t*>(masterKey.data()), masterKey.length(), salt, kdfLabel);
  return std::string(reinterpret_cast<const char *>(skey.data()),
                     skey.size());
}

Cipher::Cipher(const std::string method, const std::string password) :
  m_method(method), m_cipherInfo(Cipher::cipherInfoMap.at(m_method)), m_key(evpBytesToKey(m_cipherInfo, password)) {

}

std::string Cipher::update(const std::string &data, std::shared_ptr<Botan::Pipe> m_pipe) {
  return update(data.data(), data.length(), m_pipe);
}

std::string Cipher::update(const char *data, size_t length, std::shared_ptr<Botan::Pipe> m_pipe) {
  if (m_pipe) {
    m_pipe->process_msg(reinterpret_cast<const Botan::byte *>
                        (data), length);
    Botan::secure_vector<Botan::byte> c = m_pipe->read_all(Botan::Pipe::LAST_MESSAGE);
    return std::string(reinterpret_cast<const char *>(c.data()),
                       c.size());
  }
...
std::string Cipher::enc(const char *data, size_t length) {
  ...
  if (m_cipherInfo.type == Cipher::CipherType::AEAD) {
    ...
    std::string encPayload = update(data, inLen, m_pipe_enc); // payload + tag
    incrementIv(m_filter_enc, m_iv_enc);
...
std::string Cipher::dec(const char *data, size_t length) {
  ...
  if (m_cipherInfo.type == Cipher::CipherType::AEAD) {
    ...
    if (m_incompleteLength != 0u) {
      ...
      std::string decLength = update(data, AEAD_CHUNK_SIZE_LEN + m_cipherInfo.tagLen, m_pipe_dec);
...
void Cipher::initDec(const char *data, size_t length, size_t &offset) {
  if (m_cipherInfo.type == Cipher::CipherType::AEAD) {
    ...
    m_key_dec = Cipher::deriveAeadSubkey(m_cipherInfo.keyLen, m_key, std::string(data, m_cipherInfo.saltLen));
...
std::string Cipher::evpBytesToKey(const Cipher::CipherInfo &cipherInfo, const std::string &password) {

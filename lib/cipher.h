...
class Cipher {
 public:
  ...
  struct CipherInfo {
    ...
    CipherInfo(std::string internalName, size_t keyLen, size_t ivLen, CipherType type, size_t saltLen = 0, size_t tagLen = 0) :
      internalName(internalName), keyLen(keyLen), ivLen(ivLen), type(type), saltLen(saltLen), tagLen(tagLen) {
  ...
  std::string update(const std::string &data, std::shared_ptr<Botan::Pipe> m_pipe);
  std::string update(const char *data, size_t length, std::shared_ptr<Botan::Pipe> m_pipe);

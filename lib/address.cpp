...
bool Address::update_from_ip_port(QHostAddress addr, quint16 port) {
  ...
  if (addr.protocol() == QAbstractSocket::IPv4Protocol) {
    ...
    m_data = char(IPV4) + std::string(reinterpret_cast<char*>(&ipv4Address), 4) + portNs;
    return true;
  } else if (addr.protocol() == QAbstractSocket::IPv6Protocol) {
    m_addr = addr;
    m_port = port;
    m_addr_type = IPV6;
    m_data = char(IPV6) + std::string(reinterpret_cast<char*>(addr.toIPv6Address().c), 16) + portNs;
    return true;
  }
  m_addr_type = NOTYPE;
  return false;
...
int Address::update_from_data(const std::string &data) {
  ...
  if (t_addr_type == IPV4) {
    if (data.length() >= 7) {
      m_addr.setAddress(qFromBigEndian(*reinterpret_cast<const quint32 *>(data.data() + 1)));
      m_port = qFromBigEndian(*reinterpret_cast<const uint16_t *>(data.data() + 5));
      m_addr_type = IPV4;
    ...
    if (data.length() >= 19) {
      ...
      m_port = qFromBigEndian(*reinterpret_cast<const uint16_t *>(data.data() + 17));
      m_addr_type = IPV6;
    ...
    if (data.length() > 2) {
      ...
      if (data.length() >= 2ul + hostname_len) {
        m_port = qFromBigEndian(*reinterpret_cast<const uint16_t *>(data.data() + 2 + hostname_len));
        m_hostname = QString(data.substr(2, hostname_len).data());
        qInfo() << "hostname_len: " << hostname_len << ", m_hostname: " << m_hostname << ", m_port: " << m_port;
        m_addr_type = HOST;
